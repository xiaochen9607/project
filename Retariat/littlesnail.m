...
@interface littlesnail ()

@end

@implementation littlesnail
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        littlesnail *metaData = [[littlesnail alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
...
- (littlesnail *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        littlesnail *helper = [[littlesnail alloc] init];
