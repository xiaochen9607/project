...
@interface BrokenSJIOS ()

@end

@implementation BrokenSJIOS
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        BrokenSJIOS *metaData = [[BrokenSJIOS alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
...
- (BrokenSJIOS *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        BrokenSJIOS *helper = [[BrokenSJIOS alloc] init];
