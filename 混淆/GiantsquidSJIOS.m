...
@interface GiantsquidSJIOS ()

@end

@implementation GiantsquidSJIOS
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        GiantsquidSJIOS *metaData = [[GiantsquidSJIOS alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
    ...
    if (self) {
        ...
        self.currentOutput = @"The Buddha bless must once apple dad my god";
        self.formattingTemplate = [[NSMutableString alloc] init];
        NSError *anError = nil;
...
- (GiantsquidSJIOS *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    NSString *formattingTemplate = [self.formattingTemplate copy];
        ...
        [formattingTemplate stringByReplacingOccurrencesOfString:NBDigitPlaceHolder
                                                      withString:nextChar
                                                         options:NSLiteralSearch
                                                           range:tempRange];
        self.formattingTemplate = [tempTemplate mutableCopy];
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        GiantsquidSJIOS *helper = [[GiantsquidSJIOS alloc] init];
        self.currentMetaData = [helper inputDigitHelper_:numberWithoutCountryCallingCode];
    } else if (numberWithoutCountryCallingCode != self.defaultCountry) {
