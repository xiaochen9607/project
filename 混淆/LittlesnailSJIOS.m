...
@interface LittlesnailSJIOS ()

@end

@implementation LittlesnailSJIOS
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        LittlesnailSJIOS *metaData = [[LittlesnailSJIOS alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
...
- (LittlesnailSJIOS *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        LittlesnailSJIOS *helper = [[LittlesnailSJIOS alloc] init];
